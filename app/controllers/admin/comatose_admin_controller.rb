...
class Admin::ComatoseAdminController < Admin::BaseController
  unloadable
  
  cache_sweeper :page_sweeper, :only => [:create, :update, :destroy]
  
  include Admin::ComatoseAdminHelper

  #before_filter :handle_authorization
  before_filter :set_content_type
  
  #after_create :save_user_roles
  #before_update :save_user_roles
  ...
  # Find all pages that belong to a site and user role, based on user role access privileges setup by the admin/publisher on a parent page
  def my_pages
    @my_pages = ComatosePage.all
  end
  ...
  def edit
    #### This is for assigning pages to certain roles
    # if !cms_admin? && !role_call.include?("writer")
  ...
  def preview
    begin
      if params[:page][:id].nil?
        page = ComatosePage.new(params[:page])
      else
        page = ComatosePage.find(params[:page][:id])
        if !page.nil?
          page = page.clone
        end
        page.update_attributes(params[:page])
  ...
  def save_user_roles
    return unless params[:comatose_page]
    @user.roles.delete_all
    params[:comatose_page][:role] ||= {}
    params[:comatose_page][:role][:comatose_page] = 1     # all new accounts have user role
    Role.all.each { |role|
      @user.roles << role unless params[:comatose][:role][role.name].blank?
    }
    params[:comatose].delete(:role)
  end
